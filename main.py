...
# JSON格式化配置
app.config['JSONIFY_PRETTYPRINT_REGULAR'] = True  # 启用JSON自动格式化
...
{Fore.CYAN}====================================={Style.RESET_ALL}
{Fore.GREEN}🎟️  服务状态: {Fore.WHITE}运行中 {Fore.GREEN}●{Style.RESET_ALL}
{Fore.GREEN}📡 访问地址: {Fore.WHITE}http://{host}:{port}{Style.RESET_ALL}
{Fore.GREEN}📚 API文档: {Fore.WHITE}http://{host}:{port}{Style.RESET_ALL}
{Fore.GREEN}⚡ 示例请求: {Fore.WHITE}http://{host}:{port}/api/greeting?name=小明{Style.RESET_ALL}
...
if __name__ == '__main__':
    ...
    # 创建命令行参数解析器
    parser = argparse.ArgumentParser(description='超级个性化问候API 服务')
